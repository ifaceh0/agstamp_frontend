...
const UpdateCarousel: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log(e);
        toast.error("Failed to load carousel");
        navigate("/admin/carousels"); // ✅ FIXED: Changed from /admin/carousel to /admin/carousels
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  // ✅ Automatically determines if removing existing or new image
  const removeImage = (index: number) => {
    const isExisting = index < existingImages.length;
    ...
    <div className="max-w-2xl mx-auto mt-10 bg-white p-8 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4 pt-4">
          <button
            type="button"
            onClick={() => navigate("/admin/carousels")} // ✅ FIXED: Changed from /admin/carousel to /admin/carousels
            className="px-6 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 rounded-md transition-colors duration-200 font-medium"
          >
            Cancel
          </button>
