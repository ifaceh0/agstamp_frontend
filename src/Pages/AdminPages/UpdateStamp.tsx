...
import { useParams, useNavigate } from "react-router-dom";
import FullscreenLoader from "../../Components/Loader/FullscreenLoader";
import { useGetAllCategoriesQuery } from "../../Redux/Api/adminApi";
...
const UpdateStamp: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const initialData = {
          ...
          stock: stamp.stock,
          active: stamp.active,
          beginDate: stamp.beginDate?.slice(0, 10) || "",
          images: stamp.images,
  ...
  // ✅ FIXED: Properly determine if image is existing or new
  const removeImage = (index: number) => {
    ...
    if (isExisting) {
      ...
      console.log(`Removed new file upload`);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {previewImages.map((url, index) => (
              <div key={index} className="relative group">
                <img 
                  src={url} 
                  alt={`preview-${index}`} 
                  className="w-full h-32 object-cover rounded-md shadow-sm border-2 border-gray-200"
                />
                <button
                  type="button"
            ...
            >
              <div
                className={`w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${
                  form.active ? "transform translate-x-7" : "translate-x-1"
                }`}
              />
            </button>
          </div>
        ...
        <div className="flex justify-center space-x-4 pt-4">
          <button
            ...
            className="px-6 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg shadow-md transition-colors duration-200"
          >
            Cancel
          </button>
          <button
            type="submit"
            className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md transition-colors duration-200 disabled:bg-blue-300 disabled:cursor-not-allowed"
