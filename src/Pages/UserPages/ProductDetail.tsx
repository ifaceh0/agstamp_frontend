...
// import { useParams, useNavigate } from "react-router-dom";
// import { useGetSingleStampQuery } from "../../Redux/Api/userApi";
...
// const ProductDetail: React.FC = () => {
//   const { id } = useParams<{ id: string }>();
//   const { data, isLoading, isError } = useGetSingleStampQuery(id || "");
//   const stamp = data?.stamp;
//   const navigate = useNavigate();
//   const [addToCart, { isLoading: load }] = useAddToCartMutation();
//   const dispatch = useDispatch();
...
// import React, { useRef, useState } from "react";
// import { useParams, useNavigate } from "react-router-dom";
// // ✅ CHANGED: Import from userApi instead of adminApi
// import { useGetSingleStampQuery } from "../../Redux/Api/userApi";
// import {
//   FiShoppingCart,
//   FiTag,
...
// // ✅ Define proper interface for single stamp response
// interface StampImage {
//   publicUrl: string;
// }

// interface SingleStampData {
//   _id: string;
//   name: string;
//   description: string;
//   price: number;
...
// interface SingleStampResponse {
//   success: boolean;
//   stamp: SingleStampData;
// }

// const ProductDetail: React.FC = () => {
//   const { id } = useParams<{ id: string }>();
  
//   // ✅ CHANGED: Use the user endpoint hook with proper typing
//   const { data, isLoading, isError } = useGetSingleStampQuery(id || "", {
//     skip: !id, // Don't fetch if no ID
//   }) as { data: SingleStampResponse | undefined; isLoading: boolean; isError: boolean };
...
import { addToCartAction } from "../../Redux/Reducer/cartSlice";
import { addToGuestCart } from "../../Redux/Reducer/guestCartSlice"; // ✅ NEW
import { RootState } from "../../Redux/Store"; // ✅ NEW
import FullscreenLoader from "../../Components/Loader/FullscreenLoader";
...
const ProductDetail: React.FC = () => {
  ...
  // ✅ UPDATED: Handle both guest and logged-in users
  const handleAddToCart = async () => {
    if (isLoggedIn) {
  ...
  // ✅ NEW: Navigate to correct cart based on login status
  const handleViewCart = () => {
    navigate(isLoggedIn ? "/cart" : "/guest-cart");
  };
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="md:w-1/2 w-full p-4 sm:p-8 flex flex-col">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6">
                ...
                <div className={`p-4 rounded-xl ${stamp.stock > 0 ? "bg-green-50" : "bg-red-50"}`}>
                  <div className="flex items-center mb-1">
                    <FiPackage className={`mr-2 ${stamp.stock > 0 ? "text-green-600" : "text-red-600"}`} />
                    <span className={`font-medium ${stamp.stock > 0 ? "text-green-600" : "text-red-600"}`}>
                      Stock
                    </span>
                  </div>
                  <p className={`text-xl font-bold ${stamp.stock > 0 ? "text-gray-900" : "text-red-600"}`}>
                    {stamp.stock > 0 ? `${stamp.stock} available` : "Out of stock"}
                  </p>
                </div>
              </div>
