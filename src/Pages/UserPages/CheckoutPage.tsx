...
const CheckoutPage: React.FC = () => {
  ...
  const { cart, selectedCountry, shippingRate: reduxShippingRate } = useSelector<RootState, CartState>(
  ...
  // ✅ Use Redux values as primary source
  const shippingRate = reduxShippingRate || 0
  ...
  useEffect(() => {
    ...
    setCartTotal(subtotal + shippingRate)
  }, [cart, shippingRate])
  ...
  const handleProceedToPayment = async () => {
    try {
      ...
      if (!shippingRate || shippingRate === 0) {
        toast.error("Shipping rate is missing. Please go back to cart.")
        setIsLoading(false)
        return
      }
      ...
      const response = await createCheckoutSession({
        ...
        customerEmail: shippingAddress.email,
        customerName: shippingAddress.name,
        selectedCountry: country, // ✅ Use from Redux
        shippingType: country === "US" ? "domestic" : "international",
        shippingRate: shippingRate, // ✅ CHANGED from shippingAmount to shippingRate
        metadata: {
          products: JSON.stringify(
            cartItems.map((item) => ({
              mongoID: item.stamp._id,
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 lg:p-10">
          ...
          <div className="mb-10">
            ...
            <div className="space-y-4">
              {cartItems.map((item, index) => (
                <div key={index} className="flex justify-between items-center">
                  <div className="flex items-center">
                    {item.stamp.images && item.stamp.images[0] && (
                      <img
                        src={item.stamp.images[0].publicUrl || "/placeholder.svg"}
                        alt={item.stamp.name}
                        className="w-16 h-16 object-cover rounded-md mr-4"
          ...
          <div className="mb-10">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-gray-900 border-b pb-4">Shipping Address</h2>
              <button onClick={handleAutoFill} className="text-sm text-indigo-600 hover:text-indigo-800">
                Auto-fill for demo
              </button>
            </div>
