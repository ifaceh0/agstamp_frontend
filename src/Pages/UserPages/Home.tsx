...
//       {/* Subscription Section */}
//       <div id="subscribe" className="mt-16 flex flex-col md:flex-row items-center bg-blue-800 py-3 rounded-xl justify-center lg:justify-around w-full gap-4 px-4"
//       >
//         <div className="text-sm md:text-lg lg:text-xl w-full lg:w-2/5 text-center font-medium text-gray-100 leading-normal">
...
const Home: React.FC = () => {
  ...
  // ✅ IMPROVED: Better hash navigation handling
  useEffect(() => {
    const scrollToSubscribe = () => {
      if (window.location.hash === "#subscribe") {
        const element = document.getElementById("subscribe");
        if (element) {
          // Use smooth scroll with a slight delay to ensure DOM is ready
          setTimeout(() => {
    ...
    window.addEventListener("hashchange", scrollToSubscribe);

    return () => {
      window.removeEventListener("hashchange", scrollToSubscribe);
    };
  }, []);
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen bg-white text-center px-6 py-8">
      ...
      {/* Subscription Section - ✅ Added scroll-mt for better positioning */}
      <div 
        id="subscribe" 
        className="mt-16 flex flex-col md:flex-row items-center bg-blue-800 py-3 rounded-xl justify-center lg:justify-around w-full gap-4 px-4 scroll-mt-20"
      >
        <div className="text-sm md:text-lg lg:text-xl w-full lg:w-2/5 text-center font-medium text-gray-100 leading-normal">
          Please <span className="text-white">subscribe to our mailing list!</span>
        </div>
