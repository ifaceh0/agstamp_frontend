...
// import { useUserAllStampsQuery, useGetAllCategoriesQuery } from "../../Redux/Api/userApi";
...
const RetailSales: React.FC = () => {
  ...
  // // ✅ Categories from API
  // const { data: categoriesData = [], isLoading: isCategoriesLoading } = useGetAllCategoriesQuery();
  // const backendCategories = categoriesData.map((cat) => cat.name);

  // // ✅ Merge backend + products + "All"
  // const productCategories = products?.map((p) => p.categories) || [];
  // const categories = ["All", ...new Set([...backendCategories, ...productCategories])];

  // ✅ Get categories from the API
const { data: categoriesData = [], isLoading: isCategoriesLoading } = useGetAllCategoriesQuery();

// ✅ Create the list for the sidebar using only the category names
...
//     // If "All" is selected, we don't need to check the category.
//     if (selectedCategory === "All") {
//         return matchesSearch;
//     }

//     // 1. Find the category object that matches the selected name.
//     const categoryObject = categoriesData.find(
//         (cat) => cat.name === selectedCategory
//     );

//     // 2. Compare the product's category ID with the found category's ID. (This is the updated line)
//     const matchesCategory = categoryObject 
//         ? product.categories === categoryObject._id 
//         : false;
...
const filteredProducts = products?.filter((product) => {
  ...
  const matchesCategory = categoryObject
    ? Array.isArray(product.categories)
        ? product.categories.includes(categoryObject._id)
        : product.categories === categoryObject._id
    : false;
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row relative">
      ...
      {/* Floating Search */}
      <div
        className={`fixed top-4 right-4 z-50 transition-all duration-100 ease-in-out`}
        style={{ top: "105px" }}
      >
        {!showFloatingSearch ? (
          <button
