...
const Login = () => {
  ...
  // ✅ FIX: Add skip parameter and refetch function
  const { data: cartData, refetch: refetchCart } = useUserCartItemQuery(undefined, {
    skip: false, // Always try to fetch if user is logged in
  });
  ...
  // ✅ Sync cart data to Redux whenever it changes
  useEffect(() => {
    if (cartData) {
      dispatch(addToCartAction({ CartData: cartData, ShippingType: "" }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ FIX: Refetch cart immediately after successful login
      await refetchCart();
